...
const diagnostics=require('./cc_registry_diagnostics.cjs');
...
const signatureReport=diagnostics.diagnostics(raw,bytes);
const signatureOutput=path.join(root,'_site','cc-signature-diagnostics.json');
fs.writeFileSync(signatureOutput,JSON.stringify(signatureReport,null,2)+'\n');
console.log('CC SIGNATURE DIAGNOSTICS');
console.log(JSON.stringify(signatureReport,null,2));
